; (function () {
  ...
  swal.withForm = function () {
    ...
    $('.datepicker_calendar').datetimepicker({
      format: 'DD/MM/YYYY'
    });
  }
  ...
  extend(SwalForm.prototype, {
    ...
    getFormValues: function (isConfirm) {
      ...
      function toValuableAttrs(tag) {
        ...
        if (isConfirm && tag.required && !tag.value) {
          var warnMsg = 'Missing required attribute: ' + (tag.name || tag.id);
          tag.className += " ng-invalid";
          warnTextNode && warnTextNode.remove && warnTextNode.remove()
          warnTextNode = document.createElement("span");
          warnTextNode.className = "required";
          warnTextNode.appendChild(document.createTextNode(warnMsg))
  ...
  function Input(field) {
    var input = {
      ...
      toHtml: function () {
        ...
        switch (input.type) {
          case 'date':
            {
              inputTag = '<div class="datepicker_calendar input-group input-group-sm">';
              inputTag += t('<input type="text" id="{id}" class="form-control swal-form-field no-margin" name="{name}" value="{value}" title="{placeholder}" placeholder="{placeholder}" {required} />', input);
              inputTag += '<span class="input-group-addon">';
              inputTag += '<span class="glyphicon glyphicon-calendar"></span>';
              inputTag += '</span>';
              inputTag += '</div>';
              break;
            }
          case 'select':
            {
              inputTag = t('<select id="{id}" class="{clazz} swal-form-field" name="{name}" value="{value}" title="{placeholder}" style="width:100%" {required}>', input)
              inputTag += input.options.reduce(toHtmlOptions, '');
              inputTag += '</select>';
              break;
            }
          default: {
            inputTag = t('<input id="{id}" class="{clazz} swal-form-field" type="{type}" name="{name}" value="{value}" title="{placeholder}" placeholder="{placeholder}" {required}>', input);
            break;
          }
        }

        var row = '<div class="row_container">';
        row += input.label ? t("<label for='{id}'>{label}</label>", input) : '';
        row += inputTag;
        row += '</div>';
        return row;
    ...
    input.label = input.isRadioOrCheckbox() ? (typeof field.label !== 'undefined' ? field.label : input.value) : field.label;
